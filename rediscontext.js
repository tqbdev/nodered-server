...
const util = process.env.NODE_RED_HOME
  ? require(require.resolve("@node-red/util", {
      paths: [process.env.NODE_RED_HOME],
    })).util
...
function stringify(value) {
  let hasCircular;
  let result = safeJSONStringify(value, null, null, function (k, v) {
    hasCircular = true;
  });
...
Redis.prototype.get = function (scope, key, callback) {
  ...
  try {
    ...
    const rootKeys = key
      .map((key) => util.normalisePropertyExpression(key)[0])
      .filter((key, index, self) => self.indexOf(key) === index);
    rootKeys.forEach((key) =>
      mgetArgs.push(addPrefix(this.prefix, scope, key))
    );
  ...
  try {
    ...
    for (let i = 0; i < key.length; i++) {
      ...
      if (value[i] !== undefined) {
        ...
        if (
          stringifiedContext.circular &&
          !this.knownCircularRefs[keyParts[i][0]]
        ) {
          log.warn(
            log._("context.localfilesystem.error-circular", {
              scope: keyParts[i][0],
            })
          );
...
Redis.prototype.keys = function (scope, callback) {
  ...
  scan(this.client, addPrefix(this.prefix, scope, "*"))
    .then((result) => {
      callback(
        null,
...
Redis.prototype.delete = function (scope) {
  return scan(this.client, addPrefix(this.prefix, scope, "*")).then(
    (result) => {
      if (result.length === 0) {
        return;
      } else {
